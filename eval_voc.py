...
def get_map(preds,target,VOC_CLASSES=CLASSES,threshold=0.5,use_07_metric=False,):
    ...
    for i,class_ in enumerate(VOC_CLASSES):
        ...
        print('---class {} ap {}---'.format(class_,ap))
        print('---class {} ap {}---'.format(class_,ap),file=eval_log)
        aps += [ap]
    print('---map {}---'.format(np.mean(aps)),file=eval_log)
...
def get_targets(filelist):
    #此处的标签数据来自，模型测试调用VOC数据集类别时，保存的bbox labels信息
...
def eval():
    ...
    print(time.strftime('%Y-%m-%d %H:%m',time.localtime(time.time())),file=eval_log)
    eval_log.write('*'*15+"测试集"+'*'*15+'\n')
